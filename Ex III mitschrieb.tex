...
\subsubsection{Lenard Versuch}

Aufbau (Gegenfeldmethode)
\begin{itemize}
\item Photokathode wird mit Licht bestrahlt
\item freigesetzte Elektronen werden mittels eines elektrischen Feldes (Anode) abgebremst
\item Anzahl der durchkommenden e$^-$ wird als Photostrom gemessen
\end{itemize}
Bem:
\begin{itemize}
\item freigesetzten e$^-$ haben kinetische Energie mit einer (unbekannten) Verteilung. Mit Gegenspannung $U$ wird maximale kinetische Energie bestimmt: $U_0\Leftrightarrow I_\mathrm{Photon}=0$
\end{itemize}
Beobachtung:
\begin{enumerate}[i)]
\item $U_0=\const\cdot f-\frac{W_\mathrm{A}}{e}$
\item der Photostrom ist proportional zur Intensit"at" des Lichts
\item Gegenspannung $U_0$ h"angt \underline{NICHT} von Intensit"at ab
\item es gibt eine minimale Spannung $\frac{W_\mathrm{A}}{e}$
\item Achsenabschnitt beziehungsweise Wert von $\frac{W_\mathrm{A}}{e}$ ist materialabh"angig (die Steigung jedoch nicht)
\end{enumerate}
Interpretation:

Energie des Lichts wird an e$^-$ im Metal abgegeben $\Rightarrow$ e$^-$ treten mit maximale kinetische Energie aus
%tikz 1
\[\Rightarrow eU=E_\mathrm{Licht}-W_\mathrm{A}\]
\[\Rightarrow\ \textrm{Experiment }E_\mathrm{Licht}=hf\]
Entscheidend:\\
klassisch: \[W=\frac{1}{2}E_0(E^2+c^2B^2)=\frac{1}{2}I\]
\subsubsection{Einstein's Photonenhypothese}
Licht verh"alt sich wie ein $\underbrace{\textrm{Teilchen}}_{\textrm{Phot. }E=hf}$strom\\
\[\Rightarrow eU=hf-W_\mathrm{A}\]%box me daddy
